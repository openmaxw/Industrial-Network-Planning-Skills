...
- 只能使用 CLI 正式渲染链路生成正式 HTML
- 唯一推荐入口：`PYTHONPATH=host/cli/src python3 -m planner_cli.main generate ...`
- 生成结果应包含左侧目录、结构化章节、Graphviz 拓扑图，或 Graphviz 缺失提示

**非标准使用方式**

- 先生成中间文稿，再转换成 HTML
- 直接让模型自由拼写 HTML 页面
- 让模型根据 skill 自己“模拟”一个 HTML 页面
- 结果缺少拓扑图，且也没有 Graphviz 缺失提示

以上非标准方式生成的结果，不视为正式交付件。看到这种结果，应回到 CLI 正式命令重新生成。
