...
def _render_bullets(block: BulletBlock) -> str:
    ...
    klass = 'block'
    if block.title in {'基础事实记录', '业务场景与通信记录', '系统对象与角色归属', '部署位置与协同界面', '现场观察记录', '调研发现记录', '调研关注记录', '环境与安装条件', '原始风险触发点', '架构与分层原始依据', '拓扑结构原始记录', '性能与流量线索', '地址继承与保留条件', '设备与部署原始条件'}:
        klass = 'block raw-block'
    elif block.title in {'关键设计约束'}:
...
def _render_section(section: Section) -> str:
    ...
    for block in section.raw_inputs:
        parts.append(_render_bullets(block))
    for block in section.derived_constraints:
        parts.append(_render_bullets(block))
    ...
    for block in section.design_responses:
        parts.append(_render_bullets(block))
